...
describe('generateCitationKey', () => {
  it('uses first author last name + year', () => {
    expect(generateCitationKey(rec({ authors: ['Guido van Rossum'], year: '2020' }))).toBe(
      'rossum2020',
    );
  });

  it('handles "Last, First" form', () => {
    expect(generateCitationKey(rec({ authors: ['Einstein, Albert'], year: '1905' }))).toBe(
      'einstein1905',
    );
  });

  it('falls back to editor when no authors', () => {
    expect(generateCitationKey(rec({ editors: ['Knuth, Donald'], year: '1968' }))).toBe(
      'knuth1968',
    );
  });
...
describe('formatRecordBibtex', () => {
  it('emits @book by default and joins authors with " and "', () => {
    const out = formatRecordBibtex(rec({ title: 'A Book', authors: ['A. One', 'B. Two'] }));
  ...
  it('maps document_type to the correct BibTeX entry type', () => {
    expect(formatRecordBibtex(rec({ document_type: 'Journal Article' }))).toMatch(/^@article\{/);
    expect(formatRecordBibtex(rec({ document_type: 'Book Chapter' }))).toMatch(/^@incollection\{/);
    expect(formatRecordBibtex(rec({ document_type: 'Thesis' }))).toMatch(/^@phdthesis\{/);
  });
...
describe("formatRecord('zotero')", () => {
  it('produces valid JSON with creators split into first/last', () => {
    const json = JSON.parse(
      formatRecord(rec({ title: 'X', authors: ['Guido van Rossum'] }), 'zotero'),
    );
  ...
  it('honors document_type when choosing itemType', () => {
    expect(JSON.parse(formatRecord(rec({ document_type: 'Thesis' }), 'zotero')).itemType).toBe(
      'thesis',
    );
    expect(
      JSON.parse(formatRecord(rec({ document_type: 'Book Chapter' }), 'zotero')).itemType,
    ).toBe('bookSection');
  });
...
describe("formatRecord('text' / 'json')", () => {
  it('text format includes the title', () => {
    expect(formatRecord(rec({ title: 'Hello World' }), 'text')).toContain('Title: Hello World');
  });

  it('json format round-trips and omits raw_data by default', () => {
    const json = JSON.parse(formatRecord(rec({ title: 'X', raw_data: '<xml/>' }), 'json'));
