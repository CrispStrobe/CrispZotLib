...
- [x] **4.4** IxTheo **changed the PoW scheme again** (issue #26, weekly canary failing since 2026‑07‑13): the message became `sha256(nonce + i)` (no `ts`), difficulty became a leading‑zero‑**bit** count (`DIFFICULTY_BITS = 17`) instead of a `0000` hex prefix, and the nonce is now **server‑issued and single‑use** (a locally invented nonce — or a reused `pow_token` sent without the session cookies — is rejected). Re‑worked `ixtheoPow.ts` (`parseIxTheoChallenge` + `solveIxTheoChallenge`) and replaced the cached‑token approach in `searchService.ts` with `ixtheoFetch`, which detects the challenge page, solves its nonce, and retries with the fresh token while keeping a cookie jar for the server session. **Live‑verified** (`LIVE_PROBE=1 vitest run … -t IxTheo` passes). The sibling copies were re‑worked too: CrispLib `ixtheo_library.py` and citer `lib/ixtheo_client.py` (`_solve_pow_cookie` now fetches the challenge for a server‑issued nonce, solves on leading zero bits, and sets `pow_token` on the `requests.Session`, which also carries the server's `hmac` cookie). Both **live‑verified**: `search('Habermas')` → 2080 hits.
...
- [x] **8.5 Eliminate the Python parser duplication.** DONE. Extracted `BiblioRecord` + the name/type helpers (`clean_person_name`/`_RELATOR_WORDS`, `map_dc_type`, `infer_document_type`) + the three SRU parsers (`parse_dublin_core`/`parse_marcxml`/`parse_rdfxml`) into a single canonical `CrispLib/sru_shared.py` (1,620 lines), byte‑synced to `citer/lib/sru_shared.py` via `sync-endpoints.sh`. Both repos now `import` those and register them on their own `SRUClient` (kept local, along with each repo's SRU search glue + BibTeX/RIS formatters); `sru_library.py` shed 2,913→1,281 lines and `sru_client.py` 2,748→1,219. The two repos run the **same parser file**, so drift is now impossible, not merely guarded (8.4 remains as a cross‑check). Verified: CrispLib 45 tests, citer 17 (parser‑parity 6 + sru contract 6 + app contract 5) — all green — plus a live smoke through the shared parsers (DNB books, hebis namespace‑less MARC, BnF AV typing). The parse bodies were confirmed dependency‑clean first (AST: they reference only `re`/`logger`/`BiblioRecord`/the helpers — the `SRUClient` refs were all `@register_parser` decorators, i.e. glue).

### 8.6 — Live 20‑record parse audit + fixes (2026‑07‑04)

Ran live 20‑record searches across every parse path (RDF/DC/MARC) and material category. **Parsing itself is solid: 20/20 parsed at every endpoint, no drops.** The audit found the leaks were in the record→Zotero handoff + one endpoint‑specific parse bug, all now fixed and guarded.

- [x] **8.6 hebis endpoint + namespace‑less MARCXML.** Added **hebis** (`http://sru.hebis.de/sru/DB=2.1`, PICA/CBS, `pica.*` — the last big German academic union not covered; title/author/isbn verified live). It returns **namespace‑LESS** MARCXML (`<record>/<datafield>` with no MARC21 namespace). The TS plugin already handled this (`getElementsByTagNameNS("*", …)` — locked in via a new `marcIndex.test.ts` case), but the **Python** parsers' prefixed XPath `.//marc:datafield` matched nothing → every field empty. Fixed CrispLib + citer `parse_marcxml` to detect the datafields' actual namespace and rebind the `marc` prefix (empty for hebis, MARC21‑slim for DNB/K10plus — no double‑count for marcxchange). hebis is http‑only → added to the 7.2 probe's cloud‑vantage skip set (like `kb`/`b3kat`).
- [x] **8.7 AV typing via `dc:type`.** The DC parser ignored `dc:type` entirely, so BnF films/audio/maps fell through to `Book` and imported as books (measured: BnF `cinéma` = 20/20 "book"). Added a shared `mapDcType`/`map_dc_type` (moving image→Video, sound→Audio, still image→Image, cartographic→Map, software/dataset) read from `dc:type` in `parseSruDublinCore` (TS) + `parse_dublin_core` (CrispLib, citer), slotted above the isbn/format fallback. Now BnF `cinéma` → `videoRecording: 2, audioRecording: 4, book: 14`. New golden case `dc_film_av_type` + `mapDcType`/`mapRecordToItemType` unit tests.
- [x] **8.8 Zotero handoff: contributors / extent / extra URLs.** `integration.ts` (and the pure `zotero` formatter) mapped only authors/editors/translators to creators, silently dropping the `contributors` list — corporate co‑authors, film crew, thesis advisors (measured: a BnF film lost 6 of 7 creators; a LoC video imported with **zero**). Now contributors → Zotero `contributor` creators (corporate names single‑field), `extent` → `numPages` for book/thesis/report, and secondary URLs (only `urls[0]` filled `url`) → the `Extra` field. Covered by `test/zoteroMapping.test.ts`.
- [x] **8.9 Z39.50 — decided against.** The Zotero plugin technically cannot (sandboxed JS is HTTPS‑only, no raw TCP/port 210), and German libraries have already migrated off it (DNB dropped Z39.50 in April 2012; SRU is the LoC successor). Every large German union is covered via SRU (DNB, ZDB, K10plus = GBV+SWB, B3Kat, ÖBV, now hebis). Z39.50 would add a heavy legacy dep (PyZ3950/yaz) for ~zero new coverage — not worth it. More German reach = more SRU endpoints, not Z39.50.
- [x] **8.10 `cleanPersonName` — BnF French names.** The name cleaner stripped only a short English/German relator list and required the life‑date paren at end‑of‑string, so BnF creators like `"Tornatore, Giuseppe (1956-....). Réalisateur"` came back **unchanged** (measured: affects most BnF records). Rewrote `cleanPersonName`/`clean_person_name` (all three repos, byte‑identical + guarded by the `dc_film_av_type` golden) to strip life‑date parentheticals **anywhere** (incl. the `(1956-....)` open form and `"(1963-.... ; actrice)"` qualifiers) and match an expanded FR/DE/EN controlled relator vocabulary (Réalisateur/Compositeur/Acteur/Scénariste/… + Herausgeber/Übersetzer/… ). Explicitly does **not** truncate ordinary period‑bearing names — `"Smith, J. R."` and `"King, Martin Luther, Jr."` survive (unit‑tested).
- [x] **8.11 OAI `dc:type` AV typing.** The OAI DC path had the same gap (Europeana/DDB feeds are full of AV & images); CrispZotLib's OAI parser ignored `dc:type` and CrispLib's left `document_type` **unset entirely**. Wired the shared `mapDcType`/`map_dc_type` into `oaiRecordParser.ts` and CrispLib `oai_pmh_library.parse_dublin_core` (the latter now also falls back through `infer_document_type` so OAI DC records are typed at all). Synthetic + replay tests: moving image→Video, sound→Audio, text+ISBN still→Book. (citer has no OAI harvest per 4.3.) Contributors from OAI already benefit from the 8.8 import‑path fix.

### 8.12 — Validate the Zotero handoff against the live schema (2026‑07‑04)

The 8.7/8.8 handoff fixes were verified only through the pure functions; the actual `integration.ts` import (`setField`/`setCreator`) was never exercised. Added `scripts/validate-zotero-schema.mjs` (`npm run validate:zotero`) that checks every field + creator type the import path sets against a **running Zotero's** live schema (`127.0.0.1:23119`). It immediately caught **two real silent‑drop bugs**, both now fixed:

- [x] **8.12a `numPages` on `report`.** 8.8 set `numPages` for book/thesis/**report**, but `report` has no `numPages` field → `setField` silently dropped it. Restricted to book/thesis (both verified to have it). Fixed in `integration.ts` + the pure `zotero` formatter.
- [x] **8.12b Invalid creator types on non‑book items.** `getID(name)` only checks the type name exists **globally**, not that it's valid for the item type — so `author`/`editor`/`translator` on a `videoRecording` (whose creators are director/castMember/…) passed the old guard, and `setCreator` then **throws and loses the whole item**. My 8.7 AV‑typing fix newly triggered this (films are now `videoRecording`, not `book`). Now the code validates via `isValidForItemType` and falls back to the item type's **primary** creator type (film author→director, audio→performer, artwork→artist, map→cartographer, software→programmer; `contributor` stays valid everywhere). `npm run validate:zotero` is green after the fix.
- [x] **8.12c Dry‑run import on live data → identifiers preserved.** The local API is read‑only (`POST /items` → "Endpoint does not support method"), so real item creation needs the loaded plugin. Next best: a **dry‑run** that fetches 100 REAL records (BnF/DNB/K10plus/ZDB/hebis), runs them through the exact `integration.ts` mapping (item type + fields + the 8.12b creator resolution), and validates every produced field/creator against Zotero's live schema. Confirmed **zero creator rejections** on real data, and surfaced a minor drop: base identifiers on types lacking them (`DOI` on a book, `ISSN` on a serial‑as‑book, `publisher` on a film) were silently lost. Fixed — `integration.ts` now rescues `DOI`/`ISSN`/`ISBN`/`publisher` into the `Extra` field (Zotero convention) when the native field is unavailable; dry‑run then clean across all 100.
- [x] **8.13 OAI name cleaning.** CrispLib's OAI `parse_dublin_core` only stripped `[Übersetzer]`‑style bracket roles, not life dates — its creator names now run through `clean_person_name` too (`"Habermas, Jürgen (1929-2026)"` → `"Habermas, Jürgen"`).
- [x] **8.14 Hardening.** `sync-check` CI now re‑clones + retries (3× / 45 s) so a shared‑file change landing in the siblings seconds after this repo no longer false‑reds; a real drift still fails all attempts. Version bumped 0.1.8 → **0.2.0** with a new `CHANGELOG.md`.
