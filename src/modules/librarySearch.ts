...
export class LibrarySearchModule {
  ...
  static init(): void {
    ...
    if (_globalThis.addon && _globalThis.addon.data.dialog) {
      try {
        if (
          _globalThis.addon.data.dialog.window &&
          !_globalThis.addon.data.dialog.window.closed
        ) {
  ...
  static async runSearch(searchParams: any): Promise<BiblioRecord[]> {
    try {
      const { protocol, endpoint, title, author, isbn, maxResults } =
        searchParams;
  ...
  static async importResults(records: BiblioRecord[]): Promise<number> {
    ...
    try {
      const importedCount =
        await LibrarySearchIntegration.importToZotero(records);
