...
export class SearchService {
  ...
  static async executeSearch(
    ...
    ) => void = ztoolkit.log, // Default logger
    ...
    try {
      switch (params.protocol.toLowerCase()) {
        case "sru": {
          ...
          if (!mainWindow) {
            throw new Error(
              "Could not get Zotero main window for DOM parsing.",
            );
          }
          // Pass the necessary objects/functions to executeSruSearch
          return await this.executeSruSearch(
            params,
            log,
            mainWindow.DOMParser,
            mainWindow.Node,
            mainWindow.XPathResult,
            mainWindow.XMLSerializer,
          );
          // --- End modification ---
        }

        case "oai": {
          const [oaiSuccess, oaiRecords, oaiTotal, oaiNextToken] =
            await this.executeOaiSearch(params, log);
          if (oaiNextToken) {
            log(
              `OAI search returned next resumption token: ${oaiNextToken.substring(0, 50)}...`,
            );
          }
  ...
  private static async executeSruSearch(
    ...
    try {
      ...
      log(
        `${logPrefix} Using SRU endpoint: ${endpointInfo.name} (${endpointId})`,
      );
      ...
      try {
        ...
        if (!query && !(endpointId === "dnb" || endpointId === "zdb")) {
          log(
            `${logPrefix} SRU query is empty for non-DNB/ZDB endpoint.`,
            "warn",
          );
        ...
        log(
          `${logPrefix} Error building SRU query: ${buildError.message}`,
          "error",
        );
      ...
      log(
        `${logPrefix} Initial schema choice (user/default): ${initialSchemaChoice || "None"}`,
      );
      ...
      if (endpointId === "dnb" || endpointId === "zdb") {
        ...
        switch (lowerCaseChoice) {
          ...
          case "oai_dc": // Allow oai_dc directly too
            ...
            log(
              `${logPrefix} Mapped '${initialSchemaChoice}' to '${schemaToUseForRequest}'`,
            );
            break;
          ...
          default:
            ...
            if (!initialSchemaChoice) {
              log(
                `${logPrefix} No schema requested or configured, using DNB default 'RDFxml'.`,
              );
              schemaToUseForRequest = "RDFxml"; // DNB's documented default
            } else {
              log(
                `${logPrefix} Unknown or unmapped schema '${initialSchemaChoice}' requested for DNB/ZDB. Falling back to DNB default 'RDFxml'.`,
                "warn",
              );
        ...
        log(
          `${logPrefix} Using schema '${schemaToUseForRequest || "(None specified, client may use its default)"}' for non-DNB/ZDB endpoint.`,
        );
      }

      log(
        `${logPrefix} Final schema for SRU request: ${schemaToUseForRequest || "(Client Default)"}`,
      );
      ...
      log(
        `${logPrefix} SRU client returned: ${totalRecords} total records, fetched ${records.length} for this page.`,
      );
      return [true, records, totalRecords];
    } catch (error: any) {
      log(
        `${logPrefix} SRU search failed for endpoint ${params.endpoint}: ${error.message}`,
        "error",
      );
  ...
  private static async executeOaiSearch(
  ...
  ): Promise<[boolean, BiblioRecord[], number, string | null]> {
    // Return success, records, total, next token
    ...
    try {
      ...
      log(
        `${logPrefix} OAI Harvest Params: set=${params.set}, prefix=${params.metadataPrefix || endpointInfo.defaultMetadataPrefix}, from=${params.from}, until=${params.until}, token=${params.resumptionToken ? "..." : "none"}`,
      );
      ...
      log(
        `${logPrefix} OAIClient returned ${records.length} records (after filtering). Estimated total: ${totalCount}. Next token: ${nextResumptionToken ? "..." : "null"}`,
      );
      ...
      log(
        `${logPrefix} OAI search error for endpoint ${params.endpoint}: ${error.message}`,
        "error",
      );
  ...
  private static async executeIxTheoSearch(
    ...
    try {
      ...
      const [parsedResults, totalCount] = this.parseIxTheoHtmlResultsPage(
        htmlText,
        log,
      );
      log(
        `IxTheo HTML Parse: Found ${parsedResults.length} items on page, Total reported: ${totalCount}`,
      );
      ...
      const detailedRecordsPromises = parsedResults.map(
        async ({ id, basicInfo }) => {
          try {
            log(
              `Attempting to fetch details for IxTheo record ${id} (format: ${params.format})`,
              "log",
            );
            const detailedRecordData = await this.fetchIxTheoRecordDetails(
              id,
              params.format,
            ...
            if (detailedRecordData) {
              ...
              log(
                `fetchIxTheoRecordDetails returned null for ${id}. Falling back to basic info.`,
                "warn",
              );
              ...
              return {
                ...
                editors: [],
                translators: [],
                contributors: [], // Assume these aren't in basic
                urls: basicInfo.urls || [],
                subjects: basicInfo.subjects || [],
                year: basicInfo.year,
                publisher_name: basicInfo.publisher_name,
                format: basicInfo.format,
                raw_data: basicInfo.raw_data,
                schema: "ixtheo-html-basic", // Indicate fallback schema
              } as BiblioRecord;
            }
          } catch (detailError: any) {
            log(
              `Error processing IxTheo record ${id}: ${detailError.message}`,
              "error",
            );
            // Return error record
            return {
              id: id,
              title: basicInfo.title || "[Error Processing Record]",
              authors: basicInfo.authors || [],
              editors: [],
              translators: [],
              contributors: [],
              ...
              publisher_name: basicInfo.publisher_name,
              format: basicInfo.format,
              raw_data: `Error: ${detailError.message}\n\n${basicInfo.raw_data || ""}`,
              schema: "ixtheo-error",
            } as BiblioRecord;
          }
        },
      ...
      const detailedRecords = (
        await Promise.all(detailedRecordsPromises)
      ).filter(
        (record): record is BiblioRecord =>
          record !== null && record.schema !== "ixtheo-error",
      ); // Filter out nulls and error records

      log(
        `IxTheo: Successfully processed ${detailedRecords.length} records overall.`,
      );
  ...
  private static buildIxTheoSearchUrl(
    ...
    if (params.title?.trim()) {
      specificFieldsCriteria.push({
        value: params.title.trim(),
        type: "Title",
      });
    }
    if (params.author?.trim()) {
      specificFieldsCriteria.push({
        value: params.author.trim(),
        type: "Author",
      });
    }
    if (params.isbn?.trim()) {
    ...
    const useSimpleAllFields =
      !useAdvancedStructure && params.allFieldsTerm?.trim();
    ...
    if (useAdvancedStructure) {
      ...
      if (params.allFieldsTerm?.trim()) {
        ztoolkit.log(
          `[buildIxTheoSearchUrl] Using advanced search. Ignoring 'All Fields' term because specific fields (Title, Author, ISN) were provided.`,
        );
      }
      ...
      queryParams.append("type", "AllFields");
      ztoolkit.log(
        `[buildIxTheoSearchUrl] No search terms provided, using default '*'.`,
      );
    }
  ...
  private static parseIxTheoHtmlResultsPage(
    ...
    try {
      ...
      if (dataTotal && /^\d+$/.test(dataTotal)) {
        ...
        const summaryElement = doc.querySelector(
          ".search-stats .js-search-stats, .resultHeader .resultcount, .search-stats .pager-text",
        ...
        if (summaryElement?.textContent) {
          ...
          const match =
            summaryText.match(/(?:of|von|de|sur)\s+(\d[\d,.]*)/i) ||
            summaryText.match(/(\d[\d,.]*)\s*$/);
          if (match && match[1]) {
            totalCount = parseInt(match[1].replace(/[,.]/g, ""), 10);
            log(`Extracted total count from summary text: ${totalCount}`);
          } else {
            log(
              `Could not extract total count number from summary text: "${summaryText}"`,
              "warn",
            );
          }
        } else {
          log(
            "Could not find any element containing total count text.",
            "warn",
          );
        }
      }
      ...
      log(
        `Found ${resultItems.length} result items on HTML page using selector 'ol.record-list > li.result'`,
      );
      ...
      resultItems.forEach((item: Element, index: number) => {
        ...
        const basicInfo: Partial<BiblioRecord> = {
          authors: [],
          subjects: [],
          urls: [],
          editors: [],
        ...
        const hiddenIdInput = item.querySelector(
          "input.hiddenId",
        ) as HTMLInputElement;
        ...
        if (authorSpan?.textContent) {
          ...
          basicInfo.authors = authorSpan.textContent
            .trim()
            .split(";")
        ...
        const formatElems = item.querySelectorAll(
          "div.result-formats span.format",
        );
        basicInfo.format =
          (Array.from(formatElems) as Element[])
      ...
      if (totalCount === 0 && results.length > 0) {
        log(
          `Total count is 0. Using parsed count (${results.length}) as estimate.`,
          "warn",
        );
  ...
  private static async fetchIxTheoRecordDetails(
    ...
    try {
      ...
      if (format === "ris") {
        resultData = await this.fetchIxTheoExportData(
          recordId,
          "RIS",
          baseUrl,
          log,
        ...
        log(
          `Unsupported detail format requested: ${format}. Falling back to RIS.`,
          "warn",
        );
        resultData = await this.fetchIxTheoExportData(
          recordId,
      ...
      log(
        `Error in fetchIxTheoRecordDetails for ${recordId} (${format}): ${error.message}`,
        "error",
      );
      return null;
    }
  ...
  private static async fetchIxTheoExportData(
    ...
    try {
      ...
      if (!response.ok) {
        log(
          `Export request failed for ${recordId}: ${response.status} ${response.statusText}`,
          "error",
        );
      ...
      log(
        `Response Content-Type for ${exportFormat} export ${recordId}: ${contentType}`,
      );

      // Check for explicit error message in body
      if (
        data.includes(
          "The selected export format is not supported by this record",
        )
      ) {
        log(
          `IxTheo reported export format ${exportFormat} not supported for record ${recordId}.`,
          "warn",
        );
        return null;
      }
      ...
      if (
        ...
        (!contentType?.includes(expectedContentType) &&
          !(exportFormat === "RIS" && isPlainText))
      ) {
        log(
          `Unexpected content type received for ${exportFormat} export ${recordId}: ${contentType}. Assuming failure.`,
          "warn",
        );
        return null;
      }
      ...
      if (!data.trim()) {
        log(
          `Export returned empty response for ${recordId} (${exportFormat}).`,
          "warn",
        );
        return null;
      }
      ...
      log(
        `${exportFormat} data received and appears valid for ${recordId} (length: ${data.length})`,
      );
      return data;
    } catch (error: any) {
      log(
        `Error fetching ${exportFormat} export for ${recordId}: ${error.message}`,
        "error",
      );
      return null;
    }
  ...
  private static async fetchIxTheoDetailPage(
    ...
    try {
      ...
      log(
        `Error fetching detail page for ${recordId}: ${error.message}`,
        "error",
      );
      return null;
    }
   ...
   */
  private static parseIxTheoRis(
    risText: string,
    log: (message: string, level?: "log" | "warn" | "error") => void,
  ): Partial<BiblioRecord> {
    log(`Starting RIS parsing...`);
    const record: Partial<BiblioRecord> = {
      authors: [],
      editors: [],
      translators: [],
      urls: [],
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          switch (currentTag) {
            case "TY":
              record.format = this.mapRisTypeToFormat(currentValue);
              break;
            case "TI":
            case "T1":
              record.title = currentValue;
              break;
            case "AU":
            case "A1":
              record.authors?.push(currentValue);
              break;
            case "ED":
            case "A2":
              record.editors?.push(currentValue);
              break;
            case "A4":
              record.translators?.push(currentValue);
              break;
            case "PY":
            case "Y1": {
            ...
            case "PB":
              record.publisher_name = currentValue;
              break;
            case "CY":
              record.place_of_publication = currentValue;
              break;
            case "SN": {
              const cleanedSN = currentValue.replace(/[- ]/g, "");
              if (/^\d{4}\d{3}[\dX]$/.test(cleanedSN)) {
                record.issn = currentValue;
              } else {
                record.isbn = currentValue.replace(/\s*\(.*?\)\s*$/, "");
              }
              break;
            }
            case "T2":
            ...
            case "JF": {
              const format =
                record.format ||
                (record.issn ? "Journal Article" : "Book Chapter");
              if (format === "Journal Article") {
                record.journal_title = currentValue;
              } else {
                record.series = currentValue;
              }
              break;
            }
            case "VL":
              record.volume = currentValue;
              break;
            case "IS":
              record.issue = currentValue;
              break;
            case "SP":
              startPage = currentValue;
              break;
            case "EP":
              if (startPage && !startPage.includes("-")) {
                record.pages = `${startPage}-${currentValue}`;
              } else if (!startPage) {
                record.pages = currentValue;
              }
              break;
            case "UR":
            case "L1":
              record.urls?.push(currentValue);
              break;
            case "AB":
            case "N2":
              record.abstract = currentValue;
              break;
            case "KW":
              record.subjects?.push(currentValue);
              break;
            case "LA":
              record.language = currentValue;
              break;
            case "DO":
              record.doi = currentValue;
              break;
            case "ET":
              record.edition = currentValue;
              break;
          }
        }
      }
   ...
   * (Implementation remains the same as previously provided)
   */
  private static parseIxTheoMarc(
    marcText: string,
    log: (message: string, level?: "log" | "warn" | "error") => void,
  ): Partial<BiblioRecord> {
    ...
    try {
      const findSubfield = (
        ...
        const m = marcText.match(r);
        return m ? m[1].trim().replace(/[/\s.;,]+$/, "") : undefined;
      };
      const findSubfieldsInBlock = (
        tag: string,
        code: string,
        ind: string = "\\d\\d",
      ) => {
        const v: string[] = [];
        const fr = new RegExp(`^=${tag}\\s+${ind}\\s+.*$`, "m");
        const fm = marcText.match(fr);
        if (fm) {
          const fb = fm[0];
          const sr = new RegExp(`\\$${code}([^$]+)`, "g");
          let sm;
          while ((sm = sr.exec(fb)) !== null)
            v.push(sm[1].trim().replace(/[/\s.;,]+$/, ""));
        }
        return v;
      };

      const tA = findSubfield("245", "a");
      const tB = findSubfield("245", "b");
      record.title = tA ? (tB ? `${tA}: ${tB}` : tA) : undefined;
      const a100 = findSubfield("100", "a");
      if (a100) record.authors?.push(a100);
      record.authors?.push(...findSubfieldsInBlock("700", "a"));
      const eNames: string[] = [];
      const r700 = /^=700\s+\d\d\s+.*$/gm;
      let m700;
      while ((m700 = r700.exec(marcText)) !== null) {
        const fb = m700[0];
        const rm = fb.match(/\$e(.*?)(?:$|\$)/);
        const nm = fb.match(/\$a(.*?)(?:$|\$)/);
        if (rm && nm && rm[1].trim().match(/edt|Hrsg|hrsg|editeur|éditeur/i))
          eNames.push(nm[1].trim().replace(/[/\s.;,]+$/, ""));
      }
      record.editors = eNames;
      if (record.authors && record.editors)
        record.authors = record.authors.filter(
          (a) => !record.editors?.includes(a),
        );
      const pl = findSubfield("260", "a") || findSubfield("264", "a", ".1");
      const pub = findSubfield("260", "b") || findSubfield("264", "b", ".1");
      const date = findSubfield("260", "c") || findSubfield("264", "c", ".1");
      record.place_of_publication = pl;
      record.publisher_name = pub;
      if (date) {
        const ym = date.match(/(\d{4})/);
        record.year = ym ? ym[1] : undefined;
      }
      record.isbn = findSubfield("020", "a")?.replace(/\s*\(.*?\)\s*$/, "");
      record.issn = findSubfield("022", "a");
      record.series = findSubfield("490", "a");
      record.language = findSubfield("041", "a");
      record.subjects = findSubfieldsInBlock("650", "a");
      record.abstract = findSubfield("520", "a");
      const lm = marcText.match(/=LDR\s+(\S{24})/);
      if (lm) {
        const l = lm[1];
        const tc = l.length > 6 ? l[6] : "?";
        const lc = l.length > 7 ? l[7] : "?";
        record.format = this.mapMarcTypeToFormat(tc, lc);
      }
      const ht = findSubfield("773", "t");
      const hi = findSubfield("773", "g");
      const isA = record.format === "Journal Article" || record.issn;
      if (ht && isA) {
        record.journal_title = ht;
        if (hi) {
          const vm = hi.match(/(?:vol|v)\.?\s*(\d+)/i);
          const im = hi.match(/(?:no|nr|num)\.?\s*(\d+)/i);
          const pm = hi.match(/(?:pp|p)\.?\s*(\d+(?:-\d+)?)/i);
          record.volume = vm ? vm[1] : undefined;
      ...
      log(`Error parsing MARC data: ${error.message}`, "error");
    }
  ...
  private static mapMarcTypeToFormat(
    typeCode: string,
    levelCode: string,
  ): string | undefined {
    if (typeCode === "a") {
      if (levelCode === "m") return "Book";
      if (levelCode === "s") return "Journal";
      if (levelCode === "a") return "Journal Article";
      if (levelCode === "c") return "Book Chapter";
      if (levelCode === "i") return "Integrating Resource";
    }
    if (typeCode === "t") return "Book";
    if (typeCode === "c" || typeCode === "d") return "Music Score";
    if (typeCode === "e" || typeCode === "f") return "Map";
    if (typeCode === "g") return "Video";
    if (typeCode === "i" || typeCode === "j") return "Music Recording";
    if (typeCode === "k") return "Image";
    if (typeCode === "m") return "Computer File";
    if (typeCode === "o") return "Kit";
    if (typeCode === "p") return "Mixed Materials";
    if (typeCode === "r") return "Object";
    return undefined;
  }
   ...
   * (Implementation remains the same as previously provided)
   */
  private static parseIxTheoDetailPageHtml(
    htmlText: string,
    log: (message: string, level?: "log" | "warn" | "error") => void,
  ): Partial<BiblioRecord> {
    ...
    try {
      const win = Zotero.getMainWindow();
      if (!win || !win.DOMParser) throw new Error("DOMParser not available.");
      const parser = new win.DOMParser();
      const doc = parser.parseFromString(htmlText, "text/html");
      ...
      const ths = doc.querySelectorAll(
        ".description-tab table.table-striped th",
      );
      const findDetail = (lbl: string) => {
        for (const th of ths)
          if (th.textContent?.trim().includes(lbl))
            return (th.nextElementSibling as HTMLElement)?.textContent?.trim();
        return undefined;
      };
      const findDetailMulti = (lbl: string, sel: string = "span, a") => {
        const v: string[] = [];
        for (const th of ths)
          if (th.textContent?.trim().includes(lbl)) {
            const td = th.nextElementSibling as HTMLElement;
            if (td)
              td.querySelectorAll(sel).forEach((el: Element) => {
                const txt = el.textContent?.trim();
                if (txt && !v.includes(txt)) v.push(txt);
              });
            break;
          }
        return v;
      };
      const findUrlsInDetail = (lbl: string) => {
        const u: string[] = [];
        for (const th of ths)
          if (th.textContent?.trim().includes(lbl)) {
            const td = th.nextElementSibling as HTMLElement;
            td?.querySelectorAll("a[href]").forEach((lnk: Element) => {
              const hr = lnk.getAttribute("href");
              if (hr && hr.startsWith("http") && !u.includes(hr)) u.push(hr);
            });
            break;
          }
        return u;
      ...
      record.title = doc
        .querySelector('h3[property="name"]')
        ?.textContent?.trim();
      record.authors = findDetailMulti("Author:", 'span[property="name"]');
      record.format =
        findDetailMulti("Format:", "span.format").join(", ") || undefined;
      record.language = findDetail("Language:");
      const pubTxt = findDetail("Published:");
      if (pubTxt) {
        const ym = pubTxt.match(/(\d{4})/);
        record.year = ym ? ym[1] : undefined;
        let remTxt = pubTxt;
        if (record.year)
          remTxt = remTxt.replace(record.year, "").replace(/[,.\s]*$/, "");
        const pts = remTxt.split(":");
        if (pts.length > 1) {
          record.place_of_publication = pts[0].trim();
          record.publisher_name = pts[1].split(",")[0].trim();
        } else if (pts.length === 1 && !record.place_of_publication) {
          record.publisher_name = pts[0].trim();
        }
      }
      ...
      ths.forEach((th: Element) => {
        if (th.textContent?.trim().startsWith("Subject")) {
          const td = th.nextElementSibling as HTMLElement;
          td?.querySelectorAll("a").forEach((lnk: Element) => {
            const txt = lnk.textContent?.trim();
            if (txt) subjSet.add(txt);
          });
        }
      });
      record.subjects = Array.from(subjSet);
      record.isbn = findDetail("ISBN:");
      record.issn = findDetail("ISSN:");
      record.extent = findDetail("Physical Description:");
      record.series = findDetailMulti("Series", "a")[0];
      const jnlTxt = findDetail("In:");
      const thIn = (Array.from(ths) as Element[]).find(
        (th) => !!th.textContent?.trim().includes("In:"),
      );
      if (thIn) {
        const tdIn = thIn.nextElementSibling as HTMLElement;
        const jnlLnk = tdIn?.querySelector("a");
        record.journal_title =
          jnlLnk?.textContent?.trim() || jnlTxt?.split(",")[0].trim();
        if (jnlTxt) {
          const vm = jnlTxt.match(/Volume:\s*(\d+)/i);
          const im = jnlTxt.match(/Issue:\s*(\d+)/i);
          const pm = jnlTxt.match(/Pages:\s*(\d+(?:-\d+)?)/i);
          record.volume = vm ? vm[1] : undefined;
          record.issue = im ? im[1] : undefined;
          record.pages = pm ? pm[1] : undefined;
        }
  ...
  private static buildSruQuery(
    ...
    const formatPart = (
      ...
      if (example) {
        if (typeof example === "string") {
          if (example.includes("=")) {
            ...
            return suffixTemplate.trim().startsWith('"')
              ? `${prefix}="${trimmedValue}"`
              : `${prefix}=${trimmedValue}`;
      ...
      switch (key) {
        case "isbn":
          return endpointId === "bnf"
            ? `bib.isbn any "${trimmedValue}"`
            : `isbn=${trimmedValue}`;
        case "author":
          return endpointId === "bnf"
            ? `bib.author any "${trimmedValue}"`
            : `author="${trimmedValue}"`;
        case "title":
          return endpointId === "bnf"
            ? `bib.title any "${trimmedValue}"`
            : `title="${trimmedValue}"`;
      }
      return null;
    };
