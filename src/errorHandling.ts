  ...
  function setupErrorListeners() {
    try {
      ...
      if (targetWindow) {
        ...
        (targetWindow as any).addEventListener('error', function(event: any) {
          try {
            if (typeof Zotero !== 'undefined' && Zotero.debug) {
              ...
              Zotero.debug("[UNCAUGHT ERROR] " + message + " at " + filename + ":" + lineno);
        ...
        Zotero.debug("[LibrarySearch] Skip registering error handler - no suitable target window");
      }
    } catch (e) {
    ...
    try {
      if (typeof Zotero !== 'undefined') {
        ...
        if (Zotero.getZoteroPanes) {
          ...
          if (zoteroPanes && zoteroPanes.length > 0 && (zoteroPanes[0] as any).window) {
